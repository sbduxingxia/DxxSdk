...
import com.zhp.sdk.utils.UnitUtils;
...
//    private static final float DEFAULT_START_DEGREE = -270.0f;
    ...
    private float mStartDegree = -270.0f;

    private float mSweepDegree = 360.0f;
    ...
    private void initFromAttributes(Context context, AttributeSet attrs) {
        ...
        mStartDegree = a.getFloat(R.styleable.CircleProgressBar_progress_start_degree, mStartDegree);
        mSweepDegree = a.getFloat(R.styleable.CircleProgressBar_progress_sweep_degree, mSweepDegree);
        ...
        if (mProgressStartColor != mProgressEndColor) {
            ...
            switch (mShader) {
                ...
                case SWEEP:
                    ...
                    float rotateDegrees = (float) (mStartDegree
    ...
    private void drawLineProgress(Canvas canvas) {
        float sweepDegree = (float) ((mSweepDegree) * Math.PI / 180);
        float startDegree = (float) ((mStartDegree + 90) * Math.PI / 180);
        float unitDegrees = (float) (sweepDegree / mLineCount);
        ...
        for (int i = 0; i < mLineCount; i++) {
            float rotateDegrees = i * unitDegrees + startDegree;
    ...
    private void drawSolidProgress(Canvas canvas) {
        float sweepDegree = Math.abs(mSweepDegree);
        canvas.drawArc(mProgressRectF, mStartDegree, sweepDegree, false, mProgressBackgroundPaint);
        canvas.drawArc(mProgressRectF, mStartDegree, sweepDegree * mProgress / mMax, true, mProgressPaint);
    }
    ...
    private void drawSolidLineProgress(Canvas canvas) {
        float sweepDegree = Math.abs(mSweepDegree);
        canvas.drawArc(mProgressRectF, mStartDegree, sweepDegree, false, mProgressBackgroundPaint);
        canvas.drawArc(mProgressRectF, mStartDegree, sweepDegree * mProgress / mMax, false, mProgressPaint);
    }
